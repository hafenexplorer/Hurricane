...
public class CellarDiggingBot extends Window implements Runnable {
    ...
    public CellarDiggingBot(GameUI gui) {
        ...
        mineButton = new Button(UI.scale(110), "Start Mining") {
            @Override
            public void click() {
        ...
        activeButton = new Button(UI.scale(110), "Dig Cellar") {
            @Override
            public void click() {
                active = !active;
                if (active) {
                    this.change("Stop Digging");
                } else {
                    haltActions();
                    this.change("Start Digging");
                }
            }
        };
        add(activeButton, UI.scale(10, 190));
        pack();
    }
                ...
                if (gui.fv != null && gui.fv.current != null) {
                    ...
                    for (Gob rock : looseRocks) {
                        if (rock.rc.dist(gui.map.player().rc) < 125) {
                            ...
                            resetParams();
                            gui.msg("Loose rock in dangerous distance. Mining stopped.", Color.WHITE);
                        }
                    }
...
//                                System.out.println("build next");
    ...
    public void stop() {
        ...
        if (gui.cellarDiggingThread != null) {
            gui.cellarDiggingThread.interrupt();
